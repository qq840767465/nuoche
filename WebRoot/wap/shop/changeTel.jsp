	...
	<div id="wrapperX">
		<ul>
			<li class="main_menuAT">
				<a style="text-align: center"><span style="font-size:15px">验证商家手机号码</span></a>
			</li>
			<li>
				<ul class="fillList lineHeightA">
								...
								<li>
									...
									<span class="fill_inB"><input type="tel" placeholder="请输入" 
										name="tel" id="tel" maxlength="11" onkeyup="doTelValid(this)" value=""></span>
									<!-- <span class="fill_inC"><a onclick="changeTel()" id="btnChangeTel" class="fillList_butA"></a>11</span> -->
								</li>
								<li id="smscodePart">
									<span class="fill_inA">验证码：</span>
									<span class="fill_inB"><input type="text" placeholder="请输入" name="smscode"></span>
...
<script type="text/javascript">
	...
	function getSmsCode(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath}/ShopBind.do?p=getsmscode',
			type:'post',
			dataType:'json',
			data:'phone='+tel,
			success:function(jd){
	...
	function save(){
		...
		showPopWin(0,"正在处理，请稍候...");
		$.ajax({
			url:'${pageContext.request.contextPath}/ShopBind.do?p=savetel',
			type:'post',
			dataType:'json',
			data:p,
			success:function(jd){
				if(jd=="1"){
					showPopWin(0,"绑定成功！");
					window.location.replace("${pageContext.request.contextPath}/sjyuyuerecord.do?p=index&openid=${openid}");
				}else{
					showPopWin(1,"对不起，您还不是商家！");
				}				
			},
			error:function(){
