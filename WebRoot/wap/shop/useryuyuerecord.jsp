...
<body>
	...
	<div id="segmentedControl" class="EvaCT"
		...
		<a href="#" onclick="list(this,-1)" class="EvaCTa">所有预约服务</a> <a href="#"
			onclick="list(this,1)"  class="EvaCTb">未服务</a> <a href="#"
			onclick="list(this,2)" class="EvaCTb">已服务</a>
	</div>
	...
	<script>
			...
			function pulldownRefresh() {
				...
				if(action==1){
					...
					var status = 1;
					var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findByStatus&page=1&openid=${openid}";
					page=1;
					$('#getPage').val(1);
					...
					if (json.length == 0) { //当数据为空
						...
						for (var i = 0; i < json.length; i++) {
							...
							if (json[i].status == 1){
								m = m + '<span class="label label-primary radius" style="margin-right: -55px;" >未服务</span>';	
							}
							else if (json[i].status == 2){
								m = m + '<  span class="label label-primary radius" style="margin-right: -55px;" >已服务</span>';	
							}
							else if (json[i].status == 0){
								m = m + '<a id="'+json[i].id+'" class="userpay"  title="去支付"><span class="label label-defaunt radius">去支付</span></a>';
							...
							m = m + '<p class="mui-h6 mui-ellipsis" id="beizhu">备注：'+json[i].buyerComment+'</p>';
							m = m + '</div>';	
					...
					var status = 2;
					var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findByStatus&page=1";
					page=1;
					$('#getPage').val(1);
					var param = {status : status};
					...
					page = 1;	
					$('#getPage').val(1);	
					var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findAll&openid=${openid}&page=1";
					var param = {};
			...
			function pullupRefresh() {
				...
				if(action==1){
					...
					$('#getPage').val(page);
					var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findByStatus&page="+page;
					...
					if (jsons.sum == 0) { //当数据为空
						...
						for (var i = 0; i < json.length; i++) {
							...
							m = m + '<span class="mui-h5">';	
							if (json[i].status == 1){
								m = m + '<span class="label label-primary radius"se="margin-right: -55px;">未服务</span>';	
							}
							else if (json[i].status == 2){
								m = m + '<span class="label label-primary radius" style="margin-right: -55px;">已服务</span>';	
							}
					...
					page = $('#getPage').val()*1+1;
					$('#getPage').val(page);
					var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findAll&openid=${openid}&page="+page;
					param = {page:page};
			...
			if (action == -1) {
				...
				$('#getAction').val(-1);
				$('#getPage').val(1);
				var url = "${pageContext.request.contextPath}/useryuyueRecord.do?method=findAll&page=1";
				//${pageContext.request.contextPath }/useryuyueRecord.do?method=index&openid=${openid }
				var param = {};
		...
		<script type="text/javascript">
			...
			$(function(){
			     ...
			     mui("ul").on("tap","a",function(){
						    var id = $(this).attr("id");
						    var openid="${openid}";
			                topay(this,id,openid);
			         });
			});
			
			
			function topay(a,yuyueid,openid){
							
								var url="${pageContext.request.contextPath}/useryuyuetopay.do?p=topay&yuyueid="+yuyueid+"&openid="+openid;
