		...
		<div class="pd-20">
			<form class="form form-horizontal" id="form-article-add">
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" 
							value="" placeholder="请填写店铺名称" id="gname" name="gname" />
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-2">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" value="" placeholder="请填写店主真实姓名"
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" value="" placeholder="请填写店主电话"
				...
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text"
							name="gprice" id="gprice" placeholder="请填写店主邮箱" value=""
							class="input-text">
					</div>
				</div>
			...
			<label class="form-label col-2"> <span class="c-red">*</span>服务类型：
				...
				<div class="formControls col-2">
					<span class="select-box"> <select name="gvipprice" id="gvipprice"
				...
				<div class="row cl">
					...
					<div class="formControls col-2">
						<span class="select-box"> <select class="select"
								name="gstatus" id="gstatus" style="width: 250px;">
								<option value="0" ${b.gender==1?"selected":""} selected="selected">
									男
								</option>
				...
				<label class="form-label col-2">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" value="${barea.city.province.prName }${barea.city.ctName }" placeholder=""
							id="" name="" readonly="readonly">
					</div>
					<input type="hidden" class="input-text" value="${barea.arId }" placeholder=""
							id="arid" name="arid">
					
				<%-- <td>
					<select class="select" onchange="findByidCity()" id="prid" name="prid" style="width: 90px;float: left;height:30px;">
					...
					<select class="select" onchange="findByIdArea()" id="city" name="city" style="width: 90px;float: left;height:30px;">
						<option value="${proxy.area.city.ctId }">${proxy.area.city.ctName }</option>
					</select>
					<select class="select" id="arid" name="arid" style="width: 90px;float: left;height:30px;">
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" value="请填写店铺详细地址（详细到门牌号）" onblur="dingwei()" placeholder="请填写店铺详细地址（详细到门牌号）"
				...
				<div class="row cl">
					...
					<div class="formControls col-2">
						<span class="select-box"> <select class="select"
								name="gzhongchou" id="gzhongchou" style="width: 250px;">
								<option value="0" ${b.status==0?"selected":""} selected="selected">
									未审核
								</option>
								<option value="1" ${b.status==1?"selected":""}>
									通过
								</option>
								<option value="2" ${b.status==2?"selected":""}>
									未通过
								</option>								
							</select> </span>
					</div>
				...
				<div class="row cl" style="display: block">
					<label class="form-label col-2"><span class="c-red">*</span>店铺图片：</label>
					  <div class="formControls col-2"  id="localImag"><div class="uploader-thum-container">
							<div id="fileList" class="uploader-list"></div>
							<div class="default">
								选择图片 
							</div>
							<button id="btn-star" style="display: none;"
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<script id="editor" type="text/plain"
							style="width:100%;height:200px;" >${b.shopDesc}</script>
					</div>
				</div>
				<div class="row cl">
...
function yzgname(){
	...
	$.post(url,param,function(data){
		if(data=="true")
		{
			layer.msg("该店铺已存在",{icon:5,time:1000});
			document.getElementById("sub").disabled=true;
		}else if(data=="false")
		{
			//layer.msg("该店铺不存在!",{icon:6,time:1000});
...
/* 	if(gvipprice==null || ""==gvipprice)
	{
		layer.msg("请输入商家类型",{icon:6,time:1000});
		return false;
	} */
	...
	{
		layer.msg("请输入店铺详情",{icon:6,time:1000});
		return false;
	}
	   $.post(
	   
	      "${pageContext.request.contextPath}/daili_business.do?p=insertbusiness",
	       {
	           gid:"${b.id}",
	           gname:gname,
	           gchandi:gchandi,
...
function dingwei(){
	...
	$.ajax({
			...
			success:function(jd){
				if(jd=="1"){
					layer.msg("定位正确",{icon:6,time:1000});
				}else{
					layer.msg("详细地址错误",{icon:6,time:1000});
					
				}				
			},
			error:function(){
				layer.msg("测试失败，请稍后重试！",{icon:6,time:1000});
			}
	});	
}
