...
<![endif]-->
	...
	<body>
		...
		<div class="pd-20">
			<div class="text-c">
				<form
					action="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting"
					method="post">
					...
					<tbody>
						<c:forEach items="${lunbomap.list}" var="l" varStatus="i">
							<tr class="text-c">
								...
								<td>
									<img style="width: 120px; height: 30px;" src="${pageContext.request.contextPath}/${l.image}"/>
								</td>
								<td title="${l.title }" class="mytd">
									${l.title}
								</td>
								<!-- 将id改为overtime过期时间 -->
 								<td title="${l.overtime}" class="mytd">
									${l.overtime}
								</td> 
								<td class="td-status">
									 <c:choose>
										<c:when test="${l.type==0}">
											<span class="label label-success radius">内容 </span>
										</c:when>
										<c:when test="${l.type==1}">
											<span class="label label-success radius">跳转服务</span>
										</c:when>
										<c:otherwise>
										<span class="label label-danger radius">跳转商家</span>
										</c:otherwise>
									</c:choose>
								</td>
								<td>
									<fmt:formatDate value="${l.update}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</td>
								<td class="td-status">
									<c:choose>
								...
								<td class="td-manage">
									...
									<a title="编辑" href="${pageContext.request.contextPath}/daili_lunboimg.do?p=editlunboimgview&id=${l.id}&page=${lunbomap.page}"
							...
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									...
									<td width="32%" align="right">
										[
										<a
											href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&keywords=${keywords}"
											class="right-font08">首页</a> |
										<a
											href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.page-1}"
											class="right-font08">上一页</a> |
										<a
											href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.page+1}"
											class="right-font08">下一页</a> |
										<a
											href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.count}"
...
{
	var url="${pageContext.request.contextPath}/daili_lunboimg.do?p=updatestatus";
	var param={id:id};
...
{	
	var url="${pageContext.request.contextPath}/daili_lunboimg.do?p=deletelunbo";
	var param={id:id};
	$.post(url,param,function(data){
		 layer.confirm("确定要删除这个商品吗？",function(index){
	   window.location.href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&page=${lunbomap.page}";
...
function pldelete()
{
	 ...
	 $.ajax({
		url:"${pageContext.request.contextPath}/daili_lunboimg.do?p=pldelete",
		type:"post",
...
function plqiyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/daili_lunboimg.do?p=plupdatestatus&type=1",
		type:"post",
...
function pljinyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/daili_lunboimg.do?p=plupdatestatus&type=2",
		type:"post",
...
function viewGo()
{
	...
	if(page.match('^[0-9]*$'))
		{	
	window.location.href="${pageContext.request.contextPath}/daili_lunboimg.do?p=lunboimglisting&keywords=${keywords}&page="+page;
}
}
$(function() {
