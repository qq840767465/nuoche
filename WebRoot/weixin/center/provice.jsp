...
<html>
	...
	<body>
		<header class="mui-bar mui-bar-nav" style="background-color:#FF7900;margin-top: 0px;">
			...
			<h1 class="mui-title" style="color: white;">选择省份</h1>
		</header>	
	...
	<script type="text/javascript">
		...
		//mui.plusReady(function(){
			mui.ajax("${pageContext.request.contextPath}/"+'appaddress.do?p=getAllProvice',{
				type:'post',
				timeout:30000,
				success:function(data)
				{
					var provice=eval("("+data+")");
					var ListUl = document.getElementById("cityList");
					provice.forEach(function(provice){
						var ListEl = document.createElement("li");
							ListEl.classList.add("mui-table-view-cell");
							ListEl.innerHTML=provice.prName;
							...
							ListEl.addEventListener("click",function(){
								...
								arname = '${arname}';			
								arid = '${param.arid}';
								window.location ="${pageContext.request.contextPath}/weixin/center/insert-address.jsp?adid="+adid+"&adId="+adId+"&adname="+adname+"&adphone="+adphone+"&arid="+arid+"&arname="+arname+"&ctname="+ctname+"&prname="+provice.prName+"&addetail="+addetail+"&adstatus="+adstatus+"&prid="+provice.prId;
								//mui.fire(plus.webview.currentWebview().opener(),'showProvice',{provice:provice});
