		...
		<div class="mui-scroll">
			<div class="mui-input-group ">
				...
				<div id="ziti" class="mui-input-row mui-radio mui-left" style="display: none;">
					<label>自提</label>
					<input class="songhuoClass"  index="0" name="check" style="" onclick="corp(this,1)" id="ckbox1" type="radio">
				</div>
				<div id="kuaidi" class="mui-input-row mui-radio mui-left">
					<label>快递</label>
					<input class="songhuoClass" checked="checked"  index="1" name="check" style="" id="ckbox2" type="radio">
				</div>
				<div id="songhuo" class="mui-input-row mui-radio mui-left" style="display: none;">
					<label>送货上门</label>
					<input class="songhuoClass"  index="2" name="check" style="" onclick="corp(this,3)" id="ckbox3" type="radio">
				</div>
			</div>
			<!--地址-->
			...
			<div class="box-b">
				...
				<div style="background: #f6f6f6; padding-top:8px;padding-bottom: 3px;">
					<div class="box-b-1" id="catdiv">
						<div class="box-b-1-l" id="img"><img src="" /></div>
						<div class="box-b-1-r">
							<p id="gname">sss</p>
							<p style="width: 25%;text-align: right;">
								￥<span id="gmoney">88</span>/<span id="gdanwei"></span><br />
								<br /><span style="color:#666">
							<div class="mui-numbox" style="float: right;" data-numbox-max='4368'>
							<button id="btn-jian" class="mui-btn mui-btn-numbox-minus" type="button">-</button>
							<input id="num" onkeyup="setnum()" onclick="select()" style="color: #000000;" class="mui-input-numbox" type="number" value="1"/>
							<button id="btn-jia" class="mui-btn mui-btn-numbox-plus" type="button">+</button>
							</div>
							</span></p>
						</div>
					</div>
			...
			<div id="liuyandiv" style="background-color: white;width: 100%;height: 60px;">
				<ul style="padding-top: 15px;padding-left: 10px;">
					...
					<li style=""><input type="text" id="liuyaninput"  style="border: 1px solid #c1c1c1;width: 99%;padding-right: 10px; " /></li>
				</ul>
			</div>
			<div class="move_box">
				<div class="move">
...
<script>
	...
	//mui.plusReady(function() {
	    ...
	    //alert('<img src="${pageContext.request.contextPath}' + gimages + '" />');
	    			
		$("#img").html('<img src="${pageContext.request.contextPath}' + gimages + '" />');
		$("#gname").html(gname);
		$("#gmoney").html(gprice);
		$("#gdanwei").html(gdanwei);
		var goodslist = null; //商品list
	...
	//});
		...
		function setnum()
		{
		    ...
		    var zje = document.getElementById("order_AllPrice").innerText;
			if(num<1)
			{
				mui.toast("请输入正确的数量!");
				$("#num").val("1");
	...
	function getAllZiti(ziti) {
		var lis = '<li><a id="icon-location"><span class="mui-icon mui-icon-location"></span></a></li>' +
			'<input type="hidden" value="' + ziti.zitiId + '" id="zitiorder_id" />' +
			'<li style="padding: 10px;width: 80%;">' +
	...
	function getAllAddress(address) {
		if(address.adStatus == 1) {
			...
			var lis = '<li><a id="icon-location"><span class="mui-icon mui-icon-location"></span></a></li>' +
				...
				'</li>' +
				'<li onclick="qitaaddress()" id="qita" style="position: relative;float: right;margin-top: -40px;">其他地址<span class="mui-icon mui-icon-forward" style="color: #8F8F94;"></span></li>';
			//'<button type="button" onclick="addAddress()"	id="addaddress" style="position: absolute;width:140px;margin-left: 100px;margin-top: 77px; display:none;" class="mui-btn mui-btn-success mui-icon mui-icon-plus">添加收货地址</button>'+
			//'<li onclick="editAddress(\''+address.adId+'\',\''+address.adName+'\',\''+address.adPhone+'\',\''+address.area.arId+'\',\''+address.area.arName+'\',\''+address.area.city.ctName+'\',\''+address.area.city.provice.prName+'\',\''+address.adDetail+'\','+address.adStatus+')" id="bianji" style="position: absolute;margin-left: 280px;float: right;margin-top: 50px;display:none;">编辑<span class="mui-icon mui-icon-forward" style="color: #8F8F94;"></span></li>';
			$("#order_dizhi").html(lis);
			if(address.area.city.cityYunfei == 0) {
	...
	function qitaaddress() {
		...
		window.top.location="${pageContext.request.contextPath}/weixin/center/address.jsp?.jsp="+userid;
		/*mui.openWindow({
	...
	$("#confirm_submitorder").click(function() {
		...
		if(index == 0) {
			 adid = $("#order_id").val(); //自提地址ID
		}
		if(index == 1 || index == 2) {
			 adid = $("#order_id").val(); //地址ID
		}
		if( adid == null) {
			mui.toast("请选择收货地址");
			return;
		...
		mui.post('${pageContext.request.contextPath}/apporder.do?p=submitZhongchouOrder&type=1', {
			userid: userid,
			gid: gid,
			adid: adid,
			beizhu: beizhu,
			...
			/*mui.openWindow({
				url: "/goodsdetail/submit-order.jsp",
				id: "crowdfunding-order.jsp",
				extras: {
					gid: gid,
					num: num,
					orderId: data
