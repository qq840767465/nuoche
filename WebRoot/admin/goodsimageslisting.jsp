	...
	<body>
		<div class="cl pd-20" style="background-color: #5bacb6">
			<img class="avatar size-XL l"
				src="${pageContext.request.contextPath }/${goods.gimages}">
			<dl style="margin-left: 80px; color: #fff">
				<dt>
					<span class="f-18">${goods.gname }</span>
				</dt>
		...
		<c:if test="${type==2}">
		 ...
		 -->
				<span class="r">共有数据：<strong>${gimgmap.sum}</strong> 条</span>
		<div class="mt-20">
			<table id="tab"
				...
				<tbody>
					<c:forEach items="${gimgmap.list}" var="gi" varStatus="i">
						<tr class="text-c">
							<td>
								${i.count+(gimgmap.page-1)*gimgmap.size}
							</td>
							<td>
								<img style="width:50px; height:50px;" src="${pageContext.request.contextPath}/${gi.gimages}"/>
							</td>
							<td>
								${gi.gimgDetail}
							</td>
							<td>
								<fmt:formatDate value="${gi.gimgTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
							</td>
							<td>
									<c:choose>
										<c:when test="${gi.gimgStatus==0}"><span class="label label-defaunt radius">禁用 </span></c:when>
										<c:otherwise><span class="label label-success radius">启用</span></c:otherwise>
									</c:choose>
							</td>
							<td class="td-manage">
									<a style="text-decoration: none" onclick="xiugai_status(this,${gi.gimgId})"
									href="javascript:void(0)" title="修改商品图片状态">
										<c:choose>
										<c:when test="${gi.gimgStatus==0}"><span class="label label-success radius">启用 </span></c:when>
										<c:otherwise><span class="label label-defaunt radius">禁用</span></c:otherwise>
									</c:choose>
									</a>
								</td>
						</tr>
					...
					<td height="33">
						<table width="100%" border="0" align="center" cellpadding="0"
							...
							<tr>
								<td width="50%">
									第
									<span class="right-text09">${ gimgmap.page}</span> 页 | 共
									<span class="right-text09">${gimgmap.count }</span> 页
								</td>
								<td width="49%" align="right" id="xiugaitype">
									[
									<a
										href="${pageContext.request.contextPath}/shangjiagoods.do?p=checkGoodsImages&id=${id}"
										class="right-font08">首页</a> |
									<a
										href="${pageContext.request.contextPath}/shangjiagoods.do?p=checkGoodsImages&id=${id}&page=${gimgmap.page-1}"
										class="right-font08">上一页</a> |
									<a
										href="${pageContext.request.contextPath}/shangjiagoods.do?p=checkGoodsImages&id=${id}&page=${gimgmap.page+1}"
										class="right-font08">下一页</a> |
									<a
										href="${pageContext.request.contextPath}/shangjiagoods.do?p=checkGoodsImages&id=${id}&page=${gimgmap.count}"
										...
										<tr>
											<td width="1%">
												<input name="textfield3" type="text" id="page"
													value="${gimgmap.page}" class="right-textfield03"
													size="3" />
											</td>
											<td width="87%">
