		...
		<div class="pd-20">
			<form class="form form-horizontal" id="form-article-add">
				<div class="row cl">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" 
							value="${b.realname }" placeholder="" id="gname" name="gname" />
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-2">
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<input type="text" class="input-text" value="${b.parentId }" placeholder=""
...
<%-- 						<input type="text" class="input-text" value="${b.area }" placeholder=""
					...
					<select class="select" onchange="findByidCity()" id="prid" name="prid" style="width: 110px;float: left;height:30px;">
						<option value="${b.area.city.province.prId }">${b.area.city.province.prName }</option>
						<c:forEach items="${plist}" var="p">
							<option value="${p.prId}" ${b.area.city.province.prId==p.prId ? "selected":"" }>${p.prName}</option>
						</c:forEach>
					</select>
					<select class="select" onchange="findByIdArea()" id="city" name="city" style="width: 105px;float: left;height:30px;">
						<option value="${b.area.city.ctId }">${b.area.city.ctName }</option>
					</select>
					<select class="select" id="arid" name="arid" style="width: 100px;float: left;height:30px;">
						<option value="${b.area.arId}">${b.area.arName}</option>
					</select>
				</td>
					</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text"
							name="gprice" id="gprice" placeholder="" value="${b.openid }"
							class="input-text">
					</div>
				</div>
...
/* 	var gname=(document.getElementById("gname").value);
	...
	layer.confirm("修改商品？",function(index){
	   $.post(
	      ...
	      "${pageContext.request.contextPath}/admin_daili.do?p=updatetwo",
	       {
	           gid:"${b.id}",
	           gname:gname,
	           gchandi:gchandi,
