...
<![endif]-->
	...
	<body>
		...
		<span class="c-gray en">&gt;</span> 代理列表
		...
		<div class="pd-20">
			<div class="text-c">
					...
					<input type="text" name="keywords" class="input-text"
						style="width: 250px" value="${keywords}" placeholder="输入代理名称">
					<button type="submit" class="btn btn-success radius" >
						<i class="Hui-iconfont">&#xe665;</i> 搜索代理
					</button>
				</form>
			</div>
					...
					<tbody>
						<c:forEach items="${sjbusinessMap.list}" var="sj" varStatus="i">
							<tr class="text-c">
								...
								<td>
									<span class="label label-danger radius" >${sj.sumCard}</span>
								</td>
								
								<td>
									<span class="label label-defaunt radius">${sj.sumActive }</span>
								...
								<td>
									<a style="text-decoration: none" onclick="xiugai_status(this,${sj.id})" href="javascript:void(0)" title="修改商品状态">
										<c:if test="${sj.status ==1}"><span class="label label-danger radius">离职</span></c:if>
										<c:if test="${sj.status ==0}"><span class="label label-success radius">在职</span></c:if>
									</a>
								</td>
								<td class="td-manage">

									<a style="text-decoration: none" onclick="article_edit('编辑','admin_daili.do?p=updateone&id=${sj.id}&page=${sjgoodsMap.page}','10001')"
									href="javascript:void(0)" title="编辑">
										<span class="label label-defaunt radius">编辑 </span>
									</a>
									
									<%-- <a style="text-decoration: none" onclick="deletestatus(this,${sj.id})"
							...
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									...
									<td width="32%" align="right">
										...
										<a
											href="${pageContext.request.contextPath}/admin_daili.do?p=dalilisting&keywords=${keywords}&page=${sjbusinessMap.page-1}"
											class="right-font08">上一页</a> |
										<a
											href="${pageContext.request.contextPath}/admin_daili.do?p=dalilisting&keywords=${keywords}&page=${sjbusinessMap.page+1}"
											class="right-font08">下一页</a> |
										<a
											href="${pageContext.request.contextPath}/admin_daili.do?p=dalilisting&keywords=${keywords}&page=${sjbusinessMap.count}"
...
{
	var url="${pageContext.request.contextPath}/admin_daili.do?p=xiugaistatus";
	var params={id:id};
...
{
	var url="${pageContext.request.contextPath}/admin_daili.do?p=delete";
	var params={id:id};
	$.post(url,params,function(data){
			layer.msg("修改成功",{icon:6,time:1000});
			setTimeout("go()", 800);
...
function go(){
	window.location.href="${pageContext.request.contextPath}//admin_daili.do?p=dalilisting";
}
//批量下架
function plxiajia()
{
	...
	layer.confirm("确定要离职吗",function(index){
		$.ajax({
		url:"${pageContext.request.contextPath}/admin_daili.do?p=plxiajiagoods",
		type:"post",
