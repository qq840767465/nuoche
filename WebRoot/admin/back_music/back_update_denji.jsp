...
<html>
	...
	<body>
		...
		<article class="page-container">
			...
			<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 歌曲管理 <span class="c-gray en">&gt;</span> 编辑歌曲等级</nav>
		...
		<form action="" method="post" class="form form-horizontal">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-7">
					<input type="text" class="input-text" value="${m.name }" placeholder=""
						id="name" name="name">
				</div>
			</div>
			...
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">
					<span class="c-red">*</span>折扣:
				</label>
				<div class="formControls col-xs-8 col-sm-7">
					<input type="text" class="input-text" value="${m.zhekou }" placeholder="请输入0-1间小数"
						id="zhekou" name="zhekou">
				</div>
			</div>
...
function add_lunbo(){
	...
	if (zhekou=="" || $.trim(zhekou).length==0) {
		layer.msg('折扣不能为空！',{icon: 5,time:2000});
		return;
	}
	if (isNaN(zhekou))
	{
	   layer.msg('折扣只能是数字！',{icon: 5,time:2000});
	   return;
	}
	
	if (1*zhekou<0 || 1*zhekou>1)
	{
	   layer.msg('折扣只能是0-1间！',{icon: 5,time:2000});
	   return;
	}
	
	$.ajax({
		url: '${pageContext.request.contextPath }/musicBack.do?p=updateDenji',
		type:'POST',
		data: {name:name,id:${m.id},zhekou:zhekou},
		success: function(data){
			layer.msg('修改成功！',{icon: 6,time:2000});
			window.location.href="${pageContext.request.contextPath }/musicBack.do?p=showDenji&x="+Math.random();
