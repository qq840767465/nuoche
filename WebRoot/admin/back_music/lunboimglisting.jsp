...
<![endif]-->
	...
	<body>
		...
		<span class="c-gray en">&gt;</span> 轮播中心
		<span class="c-gray en">&gt;</span> 轮播列表
		...
		<div class="pd-20">
			<div class="text-c">
				<form
					action="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting"
					method="post">
					<input type="text" name="keywords" class="input-text"
						style="width: 250px" value="${keywords}" placeholder="输入轮播标题">
					<button type="submit" class="btn btn-success radius" >
						<i class="Hui-iconfont">&#xe665;</i> 搜轮播
					</button>
				</form>
			</div>
				...
				<table id="tab"
					...
					<tbody>
						<c:forEach items="${lunbomap.list}" var="l" varStatus="i">
							<tr class="text-c">
								<td><input  name="check" type="checkbox" value="${l.cimgId}"></td>
								<td>
									${i.count+(lunbomap.page-1)*lunbomap.size}
								</td>
								<td>
									<img style="width: 120px; height: 30px;" src="${pageContext.request.contextPath}${l.cimages}"/>
								</td>
								<td title="${l.cimgTitle }" class="mytd">
									${l.cimgTitle}
								</td>
								<td title="${l.cimgDescribe}" class="mytd">
									<c:if test="${l.cimgType==0}">${l.cimgDescribe}</c:if>
									<c:if test="${l.cimgType==1}">${l.cimgUrl}</c:if>
									<c:if test="${l.cimgType==2}">${l.cimgUrl}</c:if>
								</td>
								<td class="td-status">
									<c:choose>
										<c:when test="${l.cimgType==0}">
											<span class="label label-success radius">图片内容 </span>
										</c:when>
										<c:when test="${l.cimgType==1}">
											<span class="label label-success radius">跳转商品</span>
										</c:when>
										<c:otherwise>
										<span class="label label-danger radius">图片链接</span>
										</c:otherwise>
									</c:choose>
								</td>
								<td>
									<fmt:formatDate value="${l.cimgTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</td>
								<td class="td-status">
									<c:choose>
										<c:when test="${l.cimgStatus==0}">
								...
								<td class="td-manage">
									<a style="text-decoration: none"
										onclick="xiugai_zcstatus(this,${l.cimgId})"
									...
									<a title="编辑" href="${pageContext.request.contextPath}/lunboimg_music.do?p=editlunboimgview&id=${l.cimgId}&page=${lunbomap.page}"
										class="ml-5" style="text-decoration: none"> <span
										class="label label-primary radius" onchange="edit()">编辑</span> </a>
									<a title="删除" href="javascript:void(0)"	onclick="deletelunbo(${l.cimgId})"
										class="ml-5" style="text-decoration: none"> <span
							...
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									...
									<td width="32%" align="right">
										[
										<a
											href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&keywords=${keywords}"
											class="right-font08">首页</a> |
										<a
											href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.page-1}"
											class="right-font08">上一页</a> |
										<a
											href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.page+1}"
											class="right-font08">下一页</a> |
										<a
											href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&keywords=${keywords}&page=${lunbomap.count}"
...
{
	var url="${pageContext.request.contextPath}/lunboimg_music.do?p=updatestatus";
	var param={id:id};
...
{	
	var url="${pageContext.request.contextPath}/lunboimg_music.do?p=deletelunbo";
	var param={id:id};
	$.post(url,param,function(data){
		 layer.confirm("确定要删除这个商品吗？",function(index){
	   window.location.href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&page=${lunbomap.page}";
...
function pldelete()
{
	 ...
	 $.ajax({
		url:"${pageContext.request.contextPath}/lunboimg_music_music.do?p=pldelete",
		type:"post",
...
function plqiyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/lunboimg_music.do?p=plupdatestatus&type=1",
		type:"post",
...
function pljinyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/lunboimg_music.do?p=plupdatestatus&type=2",
		type:"post",
...
function viewGo()
{
	...
	if(page.match('^[0-9]*$'))
		{	
	window.location.href="${pageContext.request.contextPath}/lunboimg_music.do?p=lunboimglisting&keywords=${keywords}&page="+page;
}
}
$(function() {
