...
<html>
	...
	<body>
		...
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 歌曲管理 <span class="c-gray en">&gt;</span> 修改歌曲</nav>
				...
				<div class="formControls col-xs-8 col-sm-7">
					...
					<select class="select" name="denjiId" id="denjiId">
					   <c:forEach items="${list}" var="d">
					      <option ${d.id==m.MLevel.id?"selected":""} value="${d.id}">${d.name}</option>
					   </c:forEach>
					</select>
					</span>
				</div>
...
function add_lunbo(){
	...
	$.ajax({
		url: '${pageContext.request.contextPath }/musicBack.do?p=updateMusic&id=${m.id}',
		type:'POST',
		data: $('#f').serialize(),
