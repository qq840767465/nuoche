...
<![endif]-->
	...
	<body>
		...
		<span class="c-gray en">&gt;</span> 商品所有列表
		...
		<div class="pd-20">
			<div class="text-c">
				<form
					action="${pageContext.request.contextPath}/allgoods.do?p=sjgoodslinting"
					method="post">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="r">共有数据：<strong>${sjgoodsMap.sum}</strong> 条</span>
			</div>
			<div class="mt-20">
				<table id="tab"
					...
					<tbody>
						<c:forEach items="${sjgoodsMap.list}" var="sj" varStatus="i">
							<tr class="text-c">
								<td><input  name="check" type="checkbox" value="${sj.gid}"></td>
								<td>
									${i.count+(sjgoodsMap.page-1)*sjgoodsMap.size}
								</td>
								<td>
									<a style="text-decoration: none" onclick="member_show('${sj.gname}','allgoods.do?p=checkGoodsImages&id=${sj.gid}','10001','1000','500')"
								...
								<td>
									<c:if test="${sj.gstatus==2}"><span class="label label-defaunt radius">隐藏</span></c:if>
									<c:if test="${sj.gstatus==1}"><span class="label label-success radius">上架</span></c:if>
									<c:if test="${sj.gstatus==0}"><span class="label label-defaunt radius">下架</span></c:if>
								</td>
								<td class="td-status">
									<c:choose>
										<c:when test="${sj.gziying==0}">
								...
								<td class="td-manage">
									<a style="text-decoration: none" onclick="xiugai_xianshi(this,${sj.gid})"
									href="javascript:void(0)" title="修改显示状态">
										<c:choose>
											<c:when test="${sj.gstatus==2}">
												<span class="label label-success radius">显示</span>
											</c:when>
									...
									href="javascript:void(0)" title="修改商品状态">
										<c:if test="${sj.gstatus==1}"><span class="label label-defaunt radius">下架 </span></c:if>
										<c:if test="${sj.gstatus==0}"><span class="label label-success radius">上架 </span></c:if>
									</a>
									<a style="text-decoration: none" onclick="member_show('${sj.gname}','allgoods.do?p=checkGoodsCoder&id=${sj.gid}','10001','300','300')"
									href="javascript:void(0)" title="二维码">
										<span class="label label-success radius">二维码 </span>
									</a>
									<a style="text-decoration: none" onclick="article_edit('编辑','allgoods.do?p=updateone&gid=${sj.gid}&page=${sjgoodsMap.page}','10001')"
						...
						<td height="33">
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									<td width="50%">
										第
										<span class="right-text09">${ sjgoodsMap.page}</span> 页 |
										共
										<span class="right-text09">${sjgoodsMap.count }</span> 页
									</td>
									<td width="32%" align="right">
										[
										<a
											href="${pageContext.request.contextPath}/allgoods.do?p=sjgoodslinting&gttid=${param.gttid}&minprice=${param.minprice}&maxprice=${param.maxprice}&keywords=${keywords}&gstate=${param.gstate}&keywords=${param.keywords}"
											class="right-font08">首页</a> |
										<a
											href="${pageContext.request.contextPath}/allgoods.do?p=sjgoodslinting&gttid=${param.gttid}&minprice=${param.minprice}&maxprice=${param.maxprice}&keywords=${keywords}&page=${sjgoodsMap.page-1}&gstate=${param.gstate}&keywords=${param.keywords}"
											class="right-font08">上一页</a> |
										<a
											href="${pageContext.request.contextPath}/allgoods.do?p=sjgoodslinting&gttid=${param.gttid}&minprice=${param.minprice}&maxprice=${param.maxprice}&keywords=${keywords}&page=${sjgoodsMap.page+1}&gstate=${param.gstate}&keywords=${param.keywords}"
											class="right-font08">下一页</a> |
										<a
											href="${pageContext.request.contextPath}/allgoods.do?p=sjgoodslinting&gttid=${param.gttid}&minprice=${param.minprice}&maxprice=${param.maxprice}&keywords=${keywords}&page=${sjgoodsMap.count}&gstate=${param.gstate}&keywords=${param.keywords}"
											...
											<tr>
												<td width="1%">
													<input name="textfield3" type="text" id="page"
														value="${sjgoodsMap.page}" class="right-textfield03"
														size="3" />
												</td>
												<td width="87%"></td>
												<td width="87%">
