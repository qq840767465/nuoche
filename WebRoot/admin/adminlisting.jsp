		...
		<div class="pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a
					...
					onclick="member_add('添加管理员','${pageContext.request.contextPath}/admin/addAdmin.jsp','','510')"
					class="btn btn-danger radius"><i class="Hui-iconfont">&#xe600;</i>
						添加管理员</a> </span>
				<span class="r">共有数据：<strong>${adminmap.sum}</strong> 条</span>
			</div>
			<div class="mt-20">
				<table id="tab"
					...
					<tbody>
						<c:forEach items="${adminmap.list}" var="a" varStatus="i">
							<tr class="text-c">
								<td>
									${i.count+(adminmap.page-1)*adminmap.size}
								</td>
								<td>
								<img style="width:60px;height:60px;" src="${pageContext.request.contextPath}/admin/images/${a.image}"/>
								</td>
								<td>
									${a.adminName}
								</td>
								...
								<td class="td-manage">
									<a title="重置密码" onclick="editpwd(${a.adminId})" href="javascript:void(0)"
										class="ml-5" style="text-decoration: none"> <span
										class="label label-success radius">重置密码</span> </a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
						...
						<td height="33">
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									<td width="50%">
										第
										<span class="right-text09">${ adminmap.page}</span> 页 |
										共
										<span class="right-text09">${adminmap.count }</span> 页
									</td>
									<td width="32%" align="right" id="xiugaitype">
										...
										<a
											href="${pageContext.request.contextPath}/admin.do?p=adminlisting&page=${adminmap.count}"
											...
											<tr>
												<td width="1%" style="width:22px; ">
													<input name="textfield3" type="text" id="page"
														value="${adminmap.page}" class="right-textfield03"
														size="3" />
												</td>
												<td width="87%"></td>
												<td width="87%">
