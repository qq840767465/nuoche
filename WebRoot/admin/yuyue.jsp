...
<body>
	...
	<div class="pd-20">
		<div class="text-c">
			<form
				action="${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}"
				method="post">
				...
				<input type="text" name="keywords" class="input-text"
					style="width: 250px" value="${keywords}" placeholder="请输入订单编号">
					...
					<c:forEach items="${yuyueordermap.list}" var="o" varStatus="i">
						<tr class="text-c">
							...
							<td><c:if test="${o.status==0}">
									...
									<span class="label label-success radius">已服务</span>
								</c:if></td>
						...
						<table width="100%" byuyueorder="0" align="center" cellpadding="0"
							...
							<tr>
								...
								<td width="32%" align="right">[ <a
									href="${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}&begindate=${param.begindate}&enddate=${param.enddate}&keywords=${keywords}"
									class="right-font08">首页</a> | <a
									href="${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}&begindate=${param.begindate}&enddate=${param.enddate}&keywords=${keywords}&page=${yuyueordermap.page-1}"
									class="right-font08">上一页</a> | <a
									href="${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}&begindate=${param.begindate}&enddate=${param.enddate}&keywords=${keywords}&page=${yuyueordermap.page+1}"
									class="right-font08">下一页</a> | <a
									href="${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}&begindate=${param.begindate}&enddate=${param.enddate}&keywords=${keywords}&page=${yuyueordermap.count}"
	...
	<script type="text/javascript">
		...
		function viewGo() {
			...
			if (page.match('^[0-9]*$')) {
				window.location.href = "${pageContext.request.contextPath}/admin_yuyue.do?p=yuyuelisting&status=${status}&begindate=${param.begindate}&enddate=${param.enddate}&keywords=${keywords}&page=" + page;
			}
		}
		$(function() {
