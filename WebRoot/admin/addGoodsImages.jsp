...
<body>
<div class="pd-20">
<form action="${pageContext.request.contextPath}/shangjiagoods.do?p=addGoodsImages&id=${id}&page=${page}" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
	<div class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			<label class="form-label col-2">图片状态：</label>
			<div class="formControls col-2"> <span class="select-box">
				<select class="select" id="gimgstatus" name="gimgstatus">
		...
		<div class="row cl">
			...
			<input type="file" id="gimages" name="gimages" width="150px" runat="server" onchange="javascript:setImagePreview(this,localImag,preview);" />
			  <div id="localImag">
      <img id="preview"  src="" style="width: 120px; height: 120px;" />
		</div>
		</div>
		<input type="hidden" name="gimgdetailwenzi"  id="gimgdetailwenzi"/>
		<div class="row cl" id="detail">
			<label class="form-label col-2">图片内容：</label>
			<div class="formControls col-10"> 
				<script id="editor" name="gimgdetail" type="text/plain" style="width:100%;height:200px;"></script> 
			</div>
		</div>
		<div class="row cl">
...
function add()
{
	...
	$("input[name=gimgdetailwenzi]").attr("value",editor);
	var gimages=$("#gimages").val();
	if(gimages==null || ""==gimages)
	{
	...
	var gimgdescribe=$("#gimgdetail").val();
}
//检查图片的格式是否正确,同时实现预览
