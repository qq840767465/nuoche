...
.comment-main2{
position:relative;margin-left:278px;border:1px solid #dedede;border-radius:2px;
width: 400px;
}
.comment-main2:before,.comment-main2:after{position:absolute;top:11px;left:100%;right:-16px;width:0;height:0;display:block;content:" ";border-color:transparent;border-style:solid solid outset;pointer-events:none}
.comment-main2:after{border-width:7px;border-left-color:#f8f8f8;margin-bottom:1px;margin-right:2px}
.rr{border: 1px solid transparent;float: right;}
.commentList .comment-flip .avatart {float: left;}
</style>
	<body
		...
		<div class="pd-20">
				...
				<div id="msgdetail">
						...
						<li class="item cl">
							...
							<div class="comment-main">
								...
								<div class="comment-meta">
									<a class="comment-author" href="#">${ml.userinfo.usersName}</a> 发送于
									<time title="<fmt:formatDate value="${msg.metime}" pattern="yyyy年MM月dd日  HH:mm:ss"/>" datetime="2014-08-31T03:54:20">
									<fmt:formatDate value="${ml.metime}" pattern="yyyy-MM-dd HH:mm:ss"/> 
									</time>
								</div>
								</header>
									...
									<p>
										...
										<c:choose>
										   <c:when test="${fn:substring(ml.smdetail,0,4)=='img:'}">  
										 		<img width="250" height="250" alt="" src="${pageContext.request.contextPath}/messageImages/${fn:substring(ml.smdetail,4,fn:length(ml.smdetail))}">
										   </c:when>
					...
					<ul class="commentList">
						<li class="item cl">
							...
							<a href="#"><i class="avatar size-L radius"><img alt=""
										src="${pageContext.request.contextPath}/admin/images/${ml.admin.image}">
							</i>
							</a>
							</div>
							<div class="comment-main">
								<header class="comment-header">
								<div class="comment-meta">
									<a class="comment-author" href="#">${ml.admin.adminName}</a> 回复于
									<time title="<fmt:formatDate value="${msg.metime}" pattern="yyyy年MM月dd日  HH:mm:ss"/>" datetime="2014-08-31T03:54:20">
									<fmt:formatDate value="${ml.metime}" pattern="yyyy-MM-dd HH:mm:ss"/> 
									</time>
								</div>
					...
					<input type="hidden" id="smdetail" name="smdetail" />
...
function add() {
	...
	$.ajax( {
				url : "${pageContext.request.contextPath}/uamessages.do?p=replymessages&usersid=${msg.userinfo.usersId}&meid=${msg.meid}&page=${page}&type=1",
				type : "post",
				data : {
					smdetail : editor
				},
				success : function(data) {
					...
					var str = '<ul class="commentList"><li class="item cl">'
							+ '<div style="margin-right: 380px;"><a href="#"><i class="avatar size-L radius"><img alt=""src="${pageContext.request.contextPath}/admin/images/'+msg[0].admin.image+'"></i></a></div>'
							+ '<div class="comment-main">'
							+ '<header class="comment-header">'
							+ '<div class="comment-meta">'
