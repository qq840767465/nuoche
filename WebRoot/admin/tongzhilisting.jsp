...
<![endif]-->
	...
	<body>
		...
		<span class="c-gray en">&gt;</span> 通知管理
		...
		<div class="pd-20">
			<div class="text-c">
				<form
					action="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting"
					method="post">
					<input type="text" name="keywords" class="input-text"
						style="width: 250px" value="${keywords}" placeholder="输入通知标题">
					<button type="submit" class="btn btn-success radius" >
						<i class="Hui-iconfont">&#xe665;</i> 搜通知
					</button>
				</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l"><a
					...
					class="btn btn-danger radius">
						批量删除</a></span>
				<span class="r">共有数据：<strong>${tongzhimap.sum}</strong> 条</span>
			</div>
			<div class="mt-20">
				<table id="tab"
					...
					<tbody>
						<c:forEach items="${tongzhimap.list}" var="l" varStatus="i">
							<tr class="text-c">
								<td><input  name="check" type="checkbox" value="${l.tzid}"></td>
								<td>
									${i.count+(tongzhimap.page-1)*tongzhimap.size}
								</td>
								<td>
									<img style="width: 120px; height: 30px;" src="${pageContext.request.contextPath}/${l.tzimages}"/>
								</td>
								<td title="${l.tztitle}" class="mytd">    //通知详情
									${l.tztitle}  
								</td>
								<td title="${l.tzdetail}" class="mytd">
									${l.tzdetail}
								</td>
								<td>
									<fmt:formatDate value="${l.tztime}" pattern="yyyy-MM-dd HH:mm:ss"/>
								</td>
								<td class="td-status">
									<c:choose>
										<c:when test="${l.tzstatus==0}">
								...
								<td class="td-manage">
									<a style="text-decoration: none"
										onclick="xiugai_zcstatus(this,${l.tzid})"
									...
									<a title="编辑" href="${pageContext.request.contextPath}/tongzhi.do?p=edittongzhiview&id=${l.tzid}&page=${tongzhimap.page}"
										class="ml-5" style="text-decoration: none"> <span
										class="label label-primary radius">编辑</span> </a>
									<a title="删除" href="javascript:void(0)"	onclick="deletelunbo(${l.tzid})"
										class="ml-5" style="text-decoration: none"> <span
						...
						<td height="33">
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									<td width="50%">
										第
										<span class="right-text09">${ tongzhimap.page}</span> 页 |
										共
										<span class="right-text09">${tongzhimap.count }</span> 页
									</td>
									<td width="32%" align="right">
										[
										<a
											href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&keywords=${keywords}"
											class="right-font08">首页</a> |
										<a
											href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&keywords=${keywords}&page=${tongzhimap.page-1}"
											class="right-font08">上一页</a> |
										<a
											href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&keywords=${keywords}&page=${tongzhimap.page+1}"
											class="right-font08">下一页</a> |
										<a
											href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&keywords=${keywords}&page=${tongzhimap.count}"
											...
											<tr>
												<td width="1%">
													<input name="textfield3" type="text" id="page"
														value="${tongzhimap.page}" class="right-textfield03"
														size="3" />
												</td>
												<td width="87%"></td>
												<td width="87%">
...
{
	var url="${pageContext.request.contextPath}/tongzhi.do?p=updatestatus";
	var param={id:id};
...
{	
	var url="${pageContext.request.contextPath}/tongzhi.do?p=deletetongzhi";
	var param={id:id};
	$.post(url,param,function(data){
		if(data=="true")
		{
...
function go(){
	window.location.href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&page=${tongzhimap.page}";
}
//批量删除
function pldelete()
{
	 ...
	 $.ajax({
		url:"${pageContext.request.contextPath}/tongzhi.do?p=pldelete",
		type:"post",
...
function plqiyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/tongzhi.do?p=plupdatestatus&type=1",
		type:"post",
...
function pljinyong()
{
	...
	$.ajax({
		url:"${pageContext.request.contextPath}/tongzhi.do?p=plupdatestatus&type=2",
		type:"post",
...
function viewGo()
{
	...
	if(page.match('^[0-9]*$'))
		{	
	window.location.href="${pageContext.request.contextPath}/tongzhi.do?p=tongzhilisting&keywords=${keywords}&page="+page;
}
}
$(function() {
