...
<body>
<div class="pd-20">
<form action="${pageContext.request.contextPath}/corp.do?p=editzitiaddress&id=${zitiaddress.zitiId}" method="post" class="form form-horizontal" id="form-article-add">
	<div class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			<label class="form-label col-2">省：</label>
			<div class="formControls col-2"> <span class="select-box">
				<select class="select" onchange="findByidCity()" id="prid" name="prid">
					<option value="${zitiaddress.area.city.provice.prId }">${zitiaddress.area.city.provice.prName }</option>
			...
			<div class="formControls col-2"> <span class="select-box">
				<select class="select" onchange="findByIdArea()" id="city" name="city">
					<option value="${zitiaddress.area.city.ctId}">${zitiaddress.area.city.ctName}</option>
				</select>
				</span> </div>
			<label class="form-label col-2">区/县：</label>
			<div class="formControls col-2"> <span class="select-box">
				<select class="select" id="area" name="area">
				<option value= "${zitiaddress.area.arId}">${zitiaddress.area.arName}</option>
				</select>
				</span> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-2">
				<input type="text" class="input-text"  value="${zitiaddress.zitiName }" placeholder="" id="zitiname" name="zitiname">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>电话号码：</label>
			<div class="formControls col-2">
				<input type="text" class="input-text"  value="${zitiaddress.zitiPhone }" placeholder="" id="zitiphone" name="zitiphone">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-10"> 
				<script id="editor"  type="text/plain" style="width:100%;height:200px;">${zitiaddress.zitiText }</script> 
			</div>
		</div>
		<div class="row cl">
			<div class="col-10 col-offset-2">
				<button onclick="return add()" id="sub" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 提交</button>
			</div>
		</div>
	</div>
</form>
