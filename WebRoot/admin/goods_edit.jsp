	...
	<body>
		<div class="pd-20">
			<form class="form form-horizontal" id="form-article-add">
				...
				<div class="row cl">
					<label class="form-label col-2">
						<span class="c-red">*</span>商品品牌：
					</label>
					<div class="formControls col-10">
						<input type="text" class="input-text" value="${g.gpinpai }" placeholder=""
							id="gpinpai" name="gpinpai">
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-2">
						<span class="c-red">*</span>商品简介：
					</label>
					<div class="formControls col-10">
						<input type="text" class="input-text" value="${g.jianjie }" placeholder=""
				...
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text"
							...
							name="gprice" id="gprice" placeholder="" value="${g.gprice }"
							class="input-text">
					</div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text"
							onkeyup="this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0]"
							name="gvipprice" id="gvipprice" placeholder="" value="${g.gvipprice }"
							class="input-text">
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-2">
						<span class="c-red">*</span>商品单位：
					</label>
					<div class="formControls col-4">
					<input type="text"
						onkeyup=""
						name="gdanwei" id="gdanwei" placeholder="" value="${g.gdanwei }"
						class="input-text">
				</div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-2">
						<span class="select-box"> <select class="select"
								...
								<option value="1" ${g.gstatus==1?"selected":""}>
									上架
								</option>
								<option value="0" ${g.gstatus==0?"selected":""}>
									下架
								</option>
				...
				<div class="row cl">
					...
					<div class="formControls col-10" style="margin-left: -85px;">
						<div index=0 class="fimage">
						    <c:if test="${empty g.gimages}" var="f">
						        <img
				...
				<div class="row cl">
					<label class="form-label col-2">
						<span class="c-red">*</span>商品图片：
					</label>
					<div class="formControls col-10" style="margin-left: -85px;">
						<c:if test="${empty images}" var="kong">
						...
						<c:if test="${not kong }">
							...
							<c:forEach var="img" items="${images}" varStatus="ss">

								<div index="${ss.count}" class="fimage">
									<img
										src="${pageContext.request.contextPath }${img.gimages }">
								</div>

							</c:forEach>
							<c:set var="num" value="1"></c:set>
							<c:forEach begin="${fn:length(images)+num}" end="5"
								varStatus="sta">

								<div index="${sta.index}" class="fimage">
									<img
										src="${pageContext.request.contextPath }/admin/images/add.png">
								</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-10">
						<script id="editor" type="text/plain"
							style="width:100%;height:200px;">${g.gdetail }</script>
					</div>
				</div>
				<div class="row cl">
...
{	
	...
	{
		layer.msg("请输入商品品牌",{icon:6,time:1000});
		return false;
	}
	if(gjanjie==null || ""==gjanjie)
	{
		layer.msg("请输入商品简介",{icon:6,time:1000});
		return false;
	}
	
	if(gprice==null || ""==gprice)
	{
		layer.msg("请输入普通会员价格",{icon:6,time:1000});
		return false;
	}
	if(isNaN(gprice))
	{	
		layer.msg("价格只能输入数字",{icon:6,time:1000});
		return false;
	}
	if(gvipprice==null || ""==gvipprice)
	{
		layer.msg("请输入VIP会员价格",{icon:6,time:1000});
		return false;
	}
	if(isNaN(gvipprice))
	{	
	...
	if(gdanwei==null || ""==gdanwei)
	{
		layer.msg("请选择商品单位",{icon:6,time:1000});
		return false;
	}
	...
	layer.confirm("修改商品？",function(index){
	   $.post(
	      ...
	      "${pageContext.request.contextPath}/shangjiagoods.do?p=updategoods",
	       {
	           gid:"${param.gid}",
	           gname:gname,
	           gchandi:gchandi,
...
//----------------------------------22222222-------------------------------
	...
	uploader.on( 'uploadSuccess', function( file ,response) {
		...
		$("#current").find("img").attr("src","${pageContext.request.contextPath}/tmp/"+image2);
		$("#current").find("img").css({"height":"100%","width":"100%","margin-top":"0px"});
