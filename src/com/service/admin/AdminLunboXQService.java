 ...
 * */
@Service
public class AdminLunboXQService {

	@Autowired
	HqlDAO hqlDAO;

	// 根据id得到代理的轮播记录详情
	public Map lunbolisting(int id, int size, int page, int status, Timestamp begintime, Timestamp endtime,
			String keywords) {
		...
		if (StringUtil.isNotNull(keywords) && keywords.trim().length() > 0) {
			sb.append(" and business.shopname like ?");
			paramList.add("%" + keywords + "%");
		}
		sb.append(" and proxy.id like ?");
		paramList.add( id );
		String hqlsum = "select count(*) from SylbRecord" + sb.toString();
		...
		if (page > count)
			page = count;
		String hql = "from SylbRecord" + sb.toString() + " order by created desc";
