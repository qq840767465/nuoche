...
public class DailiTixianService {
	...
	//得到某个代理提现的所有记录
		public Map dailitixianjilu(int id,int size, int page){
			StringBuffer sb=new StringBuffer(" where 1=1");
			List paramList=new ArrayList();
			String hqlsum="select count(*) from ProxyCash where proxy.id = "+id;
			int sum=(int) hqlDAO.unique(hqlsum,paramList.toArray());
			int count=sum%size==0 ? sum/size :sum/size+1;
			if(page<1) page=1;
			if(page>count) page=count;
			String hql="from ProxyCash where proxy.id ="+id+" order by time desc";
