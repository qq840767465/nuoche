...
public class DailiLunbotujiluService {
	...
	//得到某个代理的所有置顶记录
		public Map daililunbotujilu(int id,int size, int page){
			StringBuffer sb=new StringBuffer(" where 1=1");
			List paramList=new ArrayList();
			String hqlsum="select count(*) from SylbRecord where proxy.id = "+id;
			int sum=(int) hqlDAO.unique(hqlsum,paramList.toArray());
			int count=sum%size==0 ? sum/size :sum/size+1;
			if(page<1) page=1;
			if(page>count) page=count;
			String hql="from SylbRecord where proxy.id ="+id+" order by created desc";
